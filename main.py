...
# Get data from json file and Load the data to MySQL
def insert_raw_data():
  ...
  df.to_sql(name='agung_Raw_Data', con=engine, if_exists="replace", index=False)
  engine.dispose()


# Create Schema based on query in sql/query.py
...
# Insert data to table dim_province
...
# Insert data to table dim_district
...
# Insert data to table fact_province_daily
...
# Insert data to table fact_province_monthly
...
# Insert data to table fact_province_yearly
...
# Insert data to table fact_district_daily
def insert_fact_district_daily(data, dim_case):
...
# Insert data to table fact_district_monthly
...
# Insert data to table fact_district_yearly
...
def insert_raw_to_warehouse(schema):
    mysql_auth = MySQL(credential['mysql_lake'])
    engine, engine_conn = mysql_auth.connect()
    data = pd.read_sql(sql='agung_Raw_Data', con=engine)
    engine.dispose()
    ...
    fact_district_daily = insert_fact_district_daily(data, dim_case)
    ...
    fact_district_daily.to_sql('fact_district_daily', schema=schema, con=engine, index=False, if_exists='replace')
...
if __name__ == '__main__':
  insert_raw_data()
  create_star_schema(schema='agung')
  insert_raw_to_warehouse(schema='agung')